#' LambdaFRD Estimator for Fuzzy Regression Discontinuity Designs
#'
#' Implements the LambdaFRD class of generalised estimators for fuzzy RD designs that
#' have finite moments of all orders in finite samples. The standard FRD estimator 
#' (lambda = 1) lacks finite moments of all orders in finite samples, leading to instability. 
#' This function implements estimators (lambda < 1) that significantly improve finite-sample performance.
...
#' \describe{
#'   \item{tau_lambda}{Point estimate of treatment effect using LambdaFRD estimator}
#'   \item{tau_Y_lambda}{Numerator of the ratio estimator}
#'   \item{tau_D_lambda}{Denominator of the ratio estimator}
#'   \item{coefficients}{Full vector of estimated coefficients using LambdaFRD estimator}
...
#'
#' @details
#' The LambdaFRD estimator takes the form:
...
#' #' result <- LambdaFRD(Y, D, X, x0 = 0, bandwidth = h_opt, psi = 4)
#'
#' @export
